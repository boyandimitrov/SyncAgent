...
async function retrieveSchema() {
    const accessKey = '3fe43c81-f753-48fe-8afc-b79ceee88bf9';
    const apiUrl = `https://promoto.bioseek.eu/api/model?accessKey=${accessKey}`;
    ...
    try {
      ...
      // Handle the error or return a default map
      return {};
    }
}

const schema = {};
async function init(mappings) {
    const serverSchema  = await retrieveSchema();
    
    schema.indexes = {};
    for (const item of serverSchema.indexes) {
        schema.indexes[item.name] = item.indexName;
    }
}
...
async function _search(mapping, latestTimestamp, latestId) {
    ...
    const query = {
        index: schema.indexes[mapping.es_managed],
        body: body
    };
...
async function insertRows (indexName, rows) {
    try {
        const body = rows.flatMap(row => [{ index: { _index: schema.indexes[indexName] } }, row]);
...
async function getBulkData(rows) {
    ...
    for (let item of rows) {
        body.push({ index: schema.indexes["stock"] });
        body.push({
            size: 1,
            query: {
                bool: {
    ...
    let results = response.body.responses.map((res, idx) => {
        ...
        log({
            ...
            meta: {store: rows[idx].fk_store, productBase: rows[idx].fk_product}
        }) 
        return null;
    });
